    ...
    const float         standard_deviation_of_noise )
{
    ...
    const Every_Pixel_in_SearchWindow_of_Every_Frame::iterator it_s{
        Pixel_Range_Iterator{ pt_s, patch_size },
        Pixel_Range_Iterator{ {patch_size,patch_size}, patch_size },
        {0,0},
        0 };
    const NLMparams params{ patch_radius, 0, standard_deviation_of_noise };
...
{
    const auto [ frameSize, pt, pt_s, it, it_s, params ] = test_params( { 4, 4 }, { 1, 1 }, { 2, 2 }, 3, 1, 1.0f );
    ...
    FrameSequence<Color_Space_Grayscale> frameSequence(1);
    ...
    ASSERT_NEAR( res, 0.98145f, 0.00001f );
}

TEST( WeightComputerTest, BasicsSingleFrameSelf )
{
    const auto [ frameSize, pt, pt_s, it, it_s, params ] = test_params( { 4, 4 }, { 1, 1 }, { 1, 1 }, 3, 1, 1.0f );
